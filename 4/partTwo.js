...
for (let index = 0; index < processedInput.length; index++) {
    const element = processedInput[index];
    let validiation = 0;
    if (element.includes("byr")) {
        let validiateElement = element.split(" ");
        let validiateElement2;
        for (let index = 0; index < validiateElement.length; index++) {
            const element2 = validiateElement[index];
            let element2Val = element2.split("\n");
            for (let i = 0; i < element2Val.length; i++) {
                if (element2Val[i].includes("byr")) {
                    validiateElement2 = element2Val[i].split(":");
                };
            };
        };
        if (validiateElement2[1] >= 1920 && validiateElement2[1] <= 2002) {
            validiation++;
        };
    };
    if (element.includes("iyr")) {
        let validiateElement = element.split(" ");
        let validiateElement2;
        for (let index = 0; index < validiateElement.length; index++) {
            const element2 = validiateElement[index];
            let element2Val = element2.split("\n");
            for (let i = 0; i < element2Val.length; i++) {
                if (element2Val[i].includes("iyr")) {
                    validiateElement2 = element2Val[i].split(":");
                };
            };
        };
        if (validiateElement2[1] >= 2010 && validiateElement2[1] <= 2020) {
            validiation++;
        };
    };
    if (element.includes("eyr")) {
        let validiateElement = element.split(" ");
        let validiateElement2;
        for (let index = 0; index < validiateElement.length; index++) {
            const element2 = validiateElement[index];
            let element2Val = element2.split("\n");
            for (let i = 0; i < element2Val.length; i++) {
                if (element2Val[i].includes("eyr")) {
                    validiateElement2 = element2Val[i].split(":");
                };
            };
        };
        if (validiateElement2[1] >= 2020 && validiateElement2[1] <= 2030) {
            validiation++;
        };
    };
    if (element.includes("hgt")) {
        let validiateElement = element.split(" ");
        let validiateElement2;
        for (let index = 0; index < validiateElement.length; index++) {
            const element2 = validiateElement[index];
            let element2Val = element2.split("\n");
            for (let i = 0; i < element2Val.length; i++) {
                if (element2Val[i].includes("hgt")) {
                    validiateElement2 = element2Val[i].split(":");
                };
            };
        };
        const cm = /^(?<value>\d+)cm$/.exec(validiateElement2[1]);
        if (cm) {
            if ( parseInt(cm.groups.value) >= 150 && parseInt(cm.groups.value) <= 193 ) { validiation++ }
        }
        const inches = /^(?<value>\d+)in$/.exec(validiateElement2[1]);
        if (inches) {
            if ( parseInt(inches.groups.value) >= 59 && parseInt(inches.groups.value) <= 76 ) { validiation++ }
        }
        fs.appendFileSync("log.txt", `${validiateElement2}\n${validiation}\n\n`);
    };
    if (element.includes("hcl")) {
        let validiateElement = element.split(" ");
        let validiateElement2;
        for (let index = 0; index < validiateElement.length; index++) {
            const element2 = validiateElement[index];
            let element2Val = element2.split("\n");
            for (let i = 0; i < element2Val.length; i++) {
                if (element2Val[i].includes("hcl")) {
                    validiateElement2 = element2Val[i].split(":");
                };
            };
        };
        if (validiateElement2[1].includes("#")) {
            let validiateElement3 = validiateElement2[1].split("#");
            if (validiateElement3[1].match(/([/g-zG-Z/])\w+/gi) == null) {
                validiation++;
            };
        };
    };
    if (element.includes("ecl")) {
        let validiateElement = element.split(" ");
        let validiateElement2;
        for (let index = 0; index < validiateElement.length; index++) {
            const element2 = validiateElement[index];
            let element2Val = element2.split("\n");
            for (let i = 0; i < element2Val.length; i++) {
                if (element2Val[i].includes("ecl")) {
                    validiateElement2 = element2Val[i].split(":");
                };
            };
        };
        if (validiateElement2[1] == "amb" || validiateElement2[1] == "blu" || validiateElement2[1] == "brn" || validiateElement2[1] == "gry" || validiateElement2[1] == "grn" || validiateElement2[1] == "hzl" || validiateElement2[1] == "oth") {
            validiation++;
        };
    };
    if (element.includes("pid")) {
        ...
        for (let index = 0; index < validiateElement.length; index++) {
            ...
            let element2Val = element2.split("\n");
            for (let i = 0; i < element2Val.length; i++) {
                if (element2Val[i].includes("pid")) {
                    validiateElement2 = element2Val[i].split(":");
                };
            };
        };
        if (Number.isInteger(+validiateElement2[1])) {
            if (validiateElement2[1].length == 9) {
                validiation++;
            };
        };
    };
