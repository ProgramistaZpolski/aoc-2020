...
const input = `14-15 v: vdvvvvvsvvvvvfpv
...
for (let index = 0; index < processedInput.length; index++) {
    ...
    let tmp2Element = tmpElement[0].split(" ");
    let tmp3Element = tmp2Element[0].split("-");
    let re = new RegExp(tmp2Element[1], 'g');
    let strCount;
    if (tmpElement[1]) {
        if ((tmpElement[1].match(re))) {
            strCount = (tmpElement[1].match(re)).length;
        } else {
            strCount = 0;
        };
        if (strCount >= parseInt(tmp3Element[0]) && strCount <= parseInt(tmp3Element[1])) {
            count++;
        } else {
            console.log(`strCount: ${strCount}  tmpElement: ${tmpElement}   tmp2Element: ${tmp2Element}    tmp3Element: ${tmp3Element}\n`);
        }
    };
};
